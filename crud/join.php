...
include "conecta.php"; // llamamos a la conexión al servidor y la base de datos
 ...
 $consulta="select oficinas.idOficina, repVentas.nombre, clientes.lim_Credito, productos.precio from oficinas
 inner join repVentas on repVentas.idEmpleados=oficinas.idOficina
 inner join clientes on clientes.idCliente=oficinas.idOficina
 inner join productos on productos.idProducto=oficinas.idOficina 
 group by oficinas.idOficina";
 $resultado=mysql_query($consulta, $link) or die("Error en: $consulta: " . mysql_error());
 
 if ($row = mysql_fetch_array($resultado)){ 
    echo "<table border = '1'> \n"; 
    echo "<tr><td>IdOficina</td><td>Nombre</td><td>Limite de Credito</td><td>Precio</td></tr>\n"; 
    do { 
       echo "<tr><td>".$row["idOficina"]."</td><td>".$row["nombre"]."</td><td>".$row["lim_Credito"]."</td><td>".$row["precio"]."</td></tr> \n"; 
